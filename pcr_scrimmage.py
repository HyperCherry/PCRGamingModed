...
from .role import (EFFECT_BUFF, EFFECT_BUFF_BY_BT, EFFECT_DIZZINESS, EFFECT_HIT_BACK, EFFECT_JUMP, EFFECT_SKILL_CHANGE,
				   ...
				   EFFECT_HURT, EFFECT_ATTR_CHANGE, EFFECT_MOVE, EFFECT_MOVE_GOAL, EFFECT_LIFESTEAL,EFFECT_BRAMBLE,
...
class Role:
	...
	def initData(self, role_id, role_info, room_obj):
		...
		if role_data :
			...
			self.attr[Attr.SPECIAL_POINT_TEA]=0
	...
	def checkStatu(self, scrimmage):
		msg = [
			...
			f'奶茶(调试用)：{self.attr[Attr.SPECIAL_POINT_TEA]}'
		]
		if len(self.buff) != 0:
			msg.append('\nbuff效果列表:')
...
class PCRScrimmage:
	...
	def hurtCalculate(self, skill_effect, use_skill_player:Role, goal_player:Role, back_msg:List):
		...
		#生命偷取
		if EFFECT_LIFESTEAL in skill_effect:# 生命偷取
		...
		#荆棘效果
		if EFFECT_BRAMBLE in skill_effect:
			bramble_prop=skill_effect[EFFECT_BRAMBLE]
			reduce=math.floor(num*bramble_prop)
			use_skill_player.attrChange(Attr.NOW_HEALTH,-reduce)
			back_msg.append(f'{uid2card(use_skill_player.user_id, self.user_card_dict)}减少了{reduce}点生命值')
